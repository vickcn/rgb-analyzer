...
- 🔍 智能邊緣檢測 (Canny 演算法)
- 💡 色光區域識別 (HSV 色彩空間)
- 🎨 多種色度演算模式（RGB、HSV、HSL、色溫）
- 🌡️ 色溫檢測與分析（使用 McCamy's approximation）
- 📊 檢測歷史記錄
- ⚙️ 可調檢測參數
- 🟩 可拖曳/縮放的檢測範圍 ROI（矩形框）
- 💾 保存當前畫面為圖片（包含 ROI 標記）
- 💾 兩種保存：原圖與標註圖（含 ROI 與完整色彩資訊）
...
系統支援多種色度演算模式，可在檢測控制面板中切換：

- **RGB 模式**：顯示紅、綠、藍三原色數值（0-255）
- **HSV 模式**：顯示色相（0-360°）、飽和度（0-100%）、明度（0-100%）
- **HSL 模式**：顯示色相（0-360°）、飽和度（0-100%）、亮度（0-100%）
- **色溫模式**：顯示色溫值（Kelvin）及色溫描述
- **全部顯示**：可展開/收合的分組顯示所有色度資訊

### 🌡️ 色溫計算

使用 McCamy's approximation 算法計算色溫：

1. RGB → CIE XYZ 色彩空間轉換（sRGB 標準）
2. 計算色度坐標 (x, y)
3. 應用 McCamy 公式：CCT = 449n³ + 3525n² + 6823.3n + 5520.33
4. 色溫範圍：1000K（極暖光）- 25000K（極冷光）
